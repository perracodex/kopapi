...
```kotlin
response<MyResponseType>(status = HttpStatusCode.OK, contentType = setOf(
...
response<AnotherResponseType>(status = HttpStatusCode.OK, contentType = setOf(
    ContentType.Application.Json,
    ContentType.Application.Xml
)) {
...
response<MyResponseType>(status = HttpStatusCode.OK, contentType = setOf(
    ...
    addType<YetAnotherType>(contentType = setOf(ContentType.Application.Pdf))
}
```
...
- If it’s not, the composition will be ignored.
