...
2. `TypeSchemaResolver`: The core orchestrator delegating to specific resolvers based on the type characteristics.
3. `Resolvers`: Specialized components that handle different kinds of types:
   ...
   3. `CollectionResolver`: Handles collection types like `List` and `Set`, including typed arrays `Array<T>`.
...
TypeInspector initiates inspection of Kotlin type
 |
 v
TypeSchemaResolver checks the type characteristics and delegates to appropriate resolvers
```

* `TypeInspector`: Serves as the entry point, starting the inspection for a given type.
* `TypeSchemaResolver`: Analyzes the type and decides which resolver should handle it based on its characteristics.

---

### TypeSchemaResolver Decision Tree

The `TypeSchemaResolver` uses a decision tree to determine how to process a given type.
...
* Delegation: At each decision point, the `TypeSchemaResolver` either delegates to a specific resolver
...
#### Traversal with TypeSchemaResolver

- Classifier Determination: The `TypeSchemaResolver` examines the type to determine its nature (e.g., array, collection, map).
...
- Recursion: If necessary, the `TypeSchemaResolver` uses recursion to handle nested types.
...
- **CollectionResolver**
    ...
    - Process:
      ...
      - Traverses the element type using the `TypeSchemaResolver`, which may involve recursion.
...
- **MapResolver**
    ...
    - Process:
      ...
      - Traverses the value type using the `TypeSchemaResolver`.
    ...
    - Process:
        ...
        - If not already cached:
            ...
            - Traverses each property of the generic type:
                - Uses the `TypeSchemaResolver` to traverse property types, substituting type parameters as necessary.
...
- **ObjectResolver**
    ...
    - Process:
        ...
        - If not, handles it as a complex type:
          ...
          - Traverses each property recursively using the `TypeSchemaResolver`.
...
- **PropertyResolver**
    ...
    - Process:
        ...
        - For each property:
          ...
          - Traverses the property's type using the `TypeSchemaResolver`.
...
- **Example:** inspecting `Page<Employee>`
    ...
    - Process:
        - The `TypeSchemaResolver` recognizes `Page` as a generic type with type parameter `T`.
...
3. `CollectionResolver`:
   ...
   - Traverses the Element Type using `TypeSchemaResolver`.
4. `TypeSchemaResolver` (recursive call):
...
5. `ObjectResolver`:
   ...
   - Traverses each property, invoking `TypeSchemaResolver `recursively.
...
2. `CollectionResolver`:
    - Resolves the Element Type: `Employee`.
   - Traverses the Element Type using `TypeSchemaResolver`.
3. `TypeSchemaResolver` (recursive call):
    - Determines the type is an Object (`Employee`).
    - Delegates to `ObjectResolver`.
4. `ObjectResolver`:
    - Checks if `Employee` is already cached.
...
2. `MapResolver`:
   ...
   - Traverses the value type using `TypeSchemaResolver`.
3. `TypeSchemaResolver` (recursive call):
    - Determines the type is an Object `Employee`.
    - Delegates to `ObjectResolver`.
4. `ObjectResolver`:
