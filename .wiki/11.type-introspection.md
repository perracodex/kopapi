...
2. `TypeSchemaBuilder`: The core orchestrator delegating to specific resolvers based on the type characteristics.
...
TypeInspector initiates inspection of Kotlin type
 |
 v
TypeSchemaBuilder checks the type characteristics and delegates to appropriate resolvers
```

* `TypeInspector`: Serves as the entry point, starting the inspection for a given type.
* `TypeSchemaBuilder`: Analyzes the type and decides which resolver should handle it based on its characteristics.

---

### TypeSchemaBuilder Decision Tree

The `TypeSchemaBuilder` uses a decision tree to determine how to process a given type.
...
* Delegation: At each decision point, the `TypeSchemaBuilder` either delegates to a specific resolver
...
#### Traversal with TypeSchemaBuilder

- Classifier Determination: The `TypeSchemaBuilder` examines the type to determine its nature (e.g., array, collection, map).
...
- Recursion: If necessary, the `TypeSchemaBuilder` uses recursion to handle nested types.
...
- **CollectionResolver**
    ...
    - Process:
      ...
      - Traverses the element type using the `TypeSchemaBuilder`, which may involve recursion.
...
- **MapResolver**
    ...
    - Process:
      ...
      - Traverses the value type using the `TypeSchemaBuilder`.
    ...
    - Process:
        ...
        - If not already cached:
            ...
            - Traverses each property of the generic type:
                - Uses the `TypeSchemaBuilder` to traverse property types, substituting type parameters as necessary.
...
- **ObjectResolver**
    ...
    - Process:
        ...
        - If not, handles it as a complex type:
          ...
          - Traverses each property recursively using the `TypeSchemaBuilder`.
...
- **PropertyResolver**
    ...
    - Process:
        ...
        - For each property:
          ...
          - Traverses the property's type using the `TypeSchemaBuilder`.
...
- **Example:** inspecting `Page<Employee>`
    ...
    - Process:
        - The `TypeSchemaBuilder` recognizes `Page` as a generic type with type parameter `T`.
...
3. `CollectionResolver`:
   ...
   - Traverses the Element Type using `TypeSchemaBuilder`.
4. `TypeSchemaBuilder` (recursive call):
...
5. `ObjectResolver`:
   ...
   - Traverses each property, invoking `TypeSchemaBuilder `recursively.
...
2. `CollectionResolver`:
    - Resolves the Element Type: `Employee`.
   - Traverses the Element Type using `TypeSchemaBuilder`.
3. `TypeSchemaBuilder` (recursive call):
    - Determines the type is an Object (`Employee`).
    - Delegates to `ObjectResolver`.
4. `ObjectResolver`:
    - Checks if `Employee` is already cached.
...
2. `MapResolver`:
   ...
   - Traverses the value type using `TypeSchemaBuilder`.
3. `TypeSchemaBuilder` (recursive call):
    - Determines the type is an Object `Employee`.
    - Delegates to `ObjectResolver`.
4. `ObjectResolver`:
